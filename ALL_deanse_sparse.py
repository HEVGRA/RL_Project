...
dense_or_sparse = 1
theconstraint = 300
thefile = "Data/data_"+ str(thenode) +".txt"
thecost = [0,0,0,0,0]
testtime = 20
...
def data_generator():
    node = thenode
    edge = 0
    if dense_or_sparse == 1: edge = int(node*(node-1)/2 - min(3,int(node*(node-1)/12)))
    else:    edge = int(node*(node-1)/2/2)
    agent = 3
...
def algo():
    ...
    print("Node = %d, Edge = %d, Constraint = %d" % (num_node, num_edge, constraint))
    print("Lower bound       = ",int(allEdgeCost/3))                     
    print("Algo              = ",Cost)
    thecost[0] += int(allEdgeCost/3)
    thecost[1] += Cost
